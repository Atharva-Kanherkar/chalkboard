...
<p align="center">
  <a href="https://github.com/Atharva-Kanherkar/chalkboard/releases/download/v0.1.0/chalkboard-demo.mp4">
    <img src="docs/demo-cover.png" alt="chalkboard demo: how chalkboard works, generated by chalkboard" width="780">
  </a>
</p>

<p align="center">
  <em>▶️ <a href="https://github.com/Atharva-Kanherkar/chalkboard/releases/download/v0.1.0/chalkboard-demo.mp4">Watch the demo</a>: this whole video was generated by chalkboard, from a single prompt describing how chalkboard works.</em>
</p>
...
contexts and concatenates, so render wall-clock tracks the longest scene, not
the sum. Image generation runs concurrently too. Tune with `--render-concurrency`.
...
with no player support or libass-enabled `ffmpeg` required. They're built from
the narration and timed per scene. Disable with `--no-subtitles` (CLI) or
...
Every bundled track is **original and CC0**: a layered chord pad + plucked
...
spend from the API's token usage, for example around `$0.42` for 7 images.
...
depth control); Repurpose is planned. The studio can also dub one production
into multiple languages in a single run (with a per-language switcher on the
result) and burn subtitles in a different language than the narration.
...
Generated images dominate the cloud cost, so cap or disable them (`--no-images`)
...
- **Diagrams & rendering**: [RoughJS](https://roughjs.com) for the hand-drawn
  aesthetic, [Graphviz](https://graphviz.org) via
  [`@hpcc-js/wasm-graphviz`](https://github.com/hpcc-systems/hpcc-js-wasm) for
  auto-layout, an [Excalidraw](https://excalidraw.com)-inspired element model for
  SceneScript, [Playwright](https://playwright.dev) for headless Chromium
  capture, and [FFmpeg](https://ffmpeg.org) for encoding and muxing.
- **Models & voices**: [Ollama](https://ollama.com) for local LLMs and
  [Piper](https://github.com/rhasspy/piper) for local TTS, with optional
  [OpenAI](https://openai.com), [Anthropic](https://www.anthropic.com), and
  [ElevenLabs](https://elevenlabs.io). Background music can come from the
  Creative Commons [Jamendo](https://www.jamendo.com) catalogue.
- **App & tooling**: [Next.js](https://nextjs.org), [React](https://react.dev),
  [Tailwind CSS](https://tailwindcss.com), [Hono](https://hono.dev),
  [Commander](https://github.com/tj/commander.js),
  [TypeScript](https://www.typescriptlang.org),
  [tsx](https://github.com/privatenumber/tsx), [Vitest](https://vitest.dev),
  [Prettier](https://prettier.io), and [pnpm](https://pnpm.io). The bundled CC0
  music is synthesised with [SoX](https://sox.sourceforge.net) and FFmpeg.
